...
namespace SQLPrac
{
    ...
    class Program
    {
        ...
        private static string task01 = "SELECT name, area_sqkm FROM Countries " +
                                       "WHERE area_sqkm > 1000000";
        private static string task02 = "SELECT c.name, o.year, o.city FROM Olympics o " + 
                                       "JOIN Countries c ON o.country_id = c.country_id";
        private static string task03 = "SELECT name FROM Players WHERE name LIKE 'Mi%'";
        private static string task04 = "SELECT name FROM Events " + 
        ...
        private static string task06 = "SELECT c.name, count(*) FROM Result r" + 
                                       "JOIN Players p ON p.player_id = r.player_id" + 
